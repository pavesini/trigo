...
task("deploy").setAction(async (_args, hre) => {
  const HeadCross = await hre.ethers.getContractFactory("HeadCross");
  ...
  console.log(`HeadCross address: ${headcrossAddr.target}`);
  return headcrossAddr.target;
});


task("full-headcross").setAction(async (_args, hre) => {
